...
for task_key in selected_keys:
    ...
    for batch_idx, (images, labels, subject_names) in enumerate(tqdm(loader, desc=f"Processing {task}")):
        for img_idx in range(images.size(0)):  # Loop through each image in the batch
            ...
            with torch.no_grad():
                attended_a, _, output_a = model.cnn_view_a(image)
                attended_b, _, output_b = model.cnn_view_b(image)
                attended_c, _, output_c = model.cnn_view_c(image)
...
results_df.to_csv(os.path.join("output", "predictions.csv"), index=False)
