...
os.makedirs("output_gender", exist_ok=True)
os.makedirs("attentions_gender", exist_ok=True)

idx_to_label = {v: k for k, v in test_dataset.label_to_idx.items()}

for batch_idx, (images, labels, subject_names) in enumerate(tqdm(test_loader, desc="Processing Gender Test Set")):
    images = images.to(device)
    for img_idx in range(images.size(0)):
        image = images[img_idx].unsqueeze(0)  # Shape: (1, C, H, W)
        label = labels[img_idx].item()
        print(label)
        true_label = idx_to_label[label]
        print(true_label)
        subject = subject_names[img_idx]
        print(subject)

        # Get attention and prediction from View A
        with torch.no_grad():
            attended_a, logits_a, _ = model.cnn_view_a(image)
            predicted_idx = torch.argmax(logits_a).item()
            predicted_label = idx_to_label[predicted_idx]

        # Generate heatmap
        heatmap_a = process_to_heatmap(attended_a, image)

        # Extract attention map without overlay
        ...
        # Save attention map
        cv2.imwrite(os.path.join("attentions_gender", f"{subject}_gender_view_a.jpg"),
        ...
        axes[1].set_title(f"Attention (View A)\nPred: {predicted_label}")
        axes[1].axis('off')

        # Save the figure
        plt.savefig(os.path.join("output_gender", f"{subject}_gender.jpg"))
        plt.close()

print("Visualization complete. Check 'output_gender' and 'attentions_gender' folders.")